...
// PERF FIX: this used to be a `now` state + interval living directly in
// MainNav, so every second the ENTIRE header (nav items, dropdown state,
// logo button, mobile panel) re-rendered just to update the clock text.
// Isolating it here means the 1-second tick only re-renders this one
// <span>. Also memoizes the Intl.DateTimeFormat instance instead of
// rebuilding it every single render (i.e. every second).
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="relative flex items-center gap-1.5 px-3.5 py-2 text-[15px] font-medium rounded-full transition-colors"
                    style={{
                      color: active
                        ? "var(--nuvex-ink)"
                        : item.enabled
                    ...
                    onMouseEnter={(e) => {
                      if (item.enabled && !active) e.currentTarget.style.color = "var(--nuvex-ink)"
                    }}
                    onMouseLeave={(e) => {
                      if (item.enabled && !active) e.currentTarget.style.color = "var(--nuvex-slate)"
                    }}
                  >
                    {active && (
                      <motion.span
